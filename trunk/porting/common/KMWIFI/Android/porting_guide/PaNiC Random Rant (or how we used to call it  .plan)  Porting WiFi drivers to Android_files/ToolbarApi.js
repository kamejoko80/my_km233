    ...
    var _RegistredToolbars =
{
    ...
    var waitingChromeToolbars = isChrome && Array.isArray(window.conduitSmartBarsLocalIds) && window.conduitSmartBarsLocalIds.slice();

    function callReadyFunc() {
        if (!window.onTpiReadyCalled && typeof (window.onTpiReady) === 'function') {
            window.onTpiReadyCalled = true;
            window.onTpiReady();
        }
    }
    ...
    function _TPIRegisterToolbarFF(objProxy) {
        _RegistredToolbars.addToolbar(objProxy);
        if (isChrome && waitingChromeToolbars && objProxy.uniqueLocalId) {
            var idx = waitingChromeToolbars.indexOf(objProxy.uniqueLocalId);
            if (idx !== -1) {
                waitingChromeToolbars.splice(idx, 1);
            }
            if (!waitingChromeToolbars.length) {
                callReadyFunc();
            }
        }
    };
    ...
    if (window.addEventListener) {
        window.addEventListener("load", function () {
            if (isChrome) {
	    	// No chrome smart bar found, using old method of just assuming it will be ready after
		// a setTimeout (doesn't always work)
                if (!window.conduitSmartBarsLocalIds || !window.conduitSmartBarsLocalIds) {
                    setTimeout(callReadyFunc, 1500);
                } else {
		    // fallback - call ready after 12 secs in case the toolbar doesn't register	
		    setTimeout(callReadyFunc, 12000);
		}
            } else {
                callReadyFunc();
            }
        ...
        window.attachEvent("onload", callReadyFunc);
    }
    ...
    var ForceRefreshFlags =
{
    ...
    if (_TPIHelper.GetBrowser() === BROWSERS_DEF.FF && navigator.userAgent && navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
        //incase the toolbar api was loaded dynamicly - throws this event to content script to attach the proxy
        window.top.postMessage(JSON.stringify({ type: "toolbarAPIUsed" }), "*");
    }
